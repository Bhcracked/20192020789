...
const youtube = new YouTube('AIzaSyArSM9G69qNbO6jfKyK-S-LCF7fzOLRW6Q');
const hastebins = require('hastebin-gen');
const getYoutubeID = require('get-youtube-id');
const yt_api_key = "AIzaSyArSM9G69qNbO6jfKyK-S-LCF7fzOLRW6Q";
  ...
  let ms = video.durationSeconds * 1000;
	...
	if (!serverQueue) {
      ...
      repeating: false
		...
		if(!args) return msg.channel.send('Cant Find Anything!!!!');
      ...
      setTimeout(() => {//:watch: Loading... [let]
        m.edit(`:notes: Added **${song.title}**` + '(` ' + song.duration + ')`' + ` to the queue at position ` + `${serverQueue.songs.length}`);
                     if (msg.channel.type === 'dm') return msg.reply('This Command Is Not Avaible In Dm\'s :x:');   
     ...
     const w = ['./PLAYING.png'];
             ...
             fs.readFile(`${w[Math.floor(Math.random() * w.length)]}`, function (err, Background) {
                 ...
                 ctx.drawImage(ground, 0, 0, 1572, 504);
                                             ...
                                             jimp.read(url, (err, ava) => {
                                                 ...
                                                 ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
                                                     if (err) return console.log(err);
                                                             ...
                                                             ava.src = buf;
                                                             ctx.beginPath();
                                                           ctx.drawImage(ava, 1475.4, 96.5, 101, 101);
                                                                            //wl
                                                     ...
                                                     ctx.fillStyle = "#ffffff";
                                                                             ctx.fillText(`${getvalueof.username}`,521.6, 79.6);
                                                     ...
                                                     ctx.fontSize = '31px';
                                                     ctx.fillStyle = "#ffffff";
                                                                             ctx.fillText(`${song.title}`,900.6, 388.2);
                                                              
                                                              
                                                                                                     ctx.font = '30px Arial';
                                                     ctx.fontSize = '31px';
                                                     ctx.fillStyle = "#ffffff";
                                                                 ctx.fillText(`${song.duration}`,564, 466.2); 
                                                   
                                                                                                        ctx.fontSize = '31px';
                                                     ctx.fillStyle = "#ffffff";
                                                                 ctx.fillText(`${serverQueue.songs.length}`,1406.4, 490.2); 
                                           ...
                                           let url = getvalueof.displayAvatarURL.endsWith(".webp") ? getvalueof.displayAvatarURL.slice(5, -20) + ".png" : getvalueof.displayAvatarURL;
                                             jimp.read(url, (err, ava) => {
                                                 ...
                                                 ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
                                                     ...
                                                     ctx.fontSize = '31px';
                                                     ctx.fillStyle = "#ffffff";
                                                                             ctx.fillText(`${queue.volume}`,900.6, 388.2);
                                                              
                                                              
                                                                                                     ctx.font = '30px Arial';
                                                     ctx.fontSize = '31px';
                                                     ctx.fillStyle = "#ffffff";
                                                                 ctx.fillText(`( '+ args[0] +')`,140, 160); 
...
> Support Server: https://discord.gg/XGU3b33
**`
  
  if(message.content === prefix + 'help') {
            message.delete(1000)
    let e = '** Sending Help :envelope_with_arrow: **'
	  message.reply(e).then(m => m.delete(1000))
	  message.author.send(helplist).catch(error => message.reply('** Please Open DM :negative_squared_cross_mark:**'))
}
});
